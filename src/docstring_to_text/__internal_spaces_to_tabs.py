...
class _FirstExtractedSubBlock(_t.NamedTuple):
	...
	def _all_args_dict(self) -> dict:
		return dict(
			common_indent=self.common_indent,
			extracted_is_final=self.extracted_is_final,
			block_type=self.block_type,
	...
	def extract_from_pending_block(
		cls: '_FirstExtractedSubBlock', pending_lines: _t_lines_chunk
		...
		if not pending_lines:
			...
			return cls(0, True, BlockType.Indent)
		...
		if min_indent is None:
			...
			return cls(0, True, BlockType.Indent, extracted=pending_lines)
		...
		if min_indent != 0:
			...
			return cls(min_indent, False, first_real_line_type, extracted=pending_lines)
		...
		return cls(0, True, first_real_line_type, extracted=first_chunk, pending=remaining_lines)
...
class SpaceToTabsConverter(_CopyableWithOverrides):
	...
	def _all_args_dict(self) -> dict:
		return dict(
			minimize_indents=self.minimize_indents,
