...
from .__base_classes import *
from .__compat_dataclasses import *
from .__internal_enums import *


@_dataclass(**_dataclass_slots_args)
class ParsedLine(_CopyableWithOverrides):
	...
	def _all_args_dict(self) -> dict:
		...
		return _dataclass_as_dict(self)

	@classmethod
	def unindent(cls, lines: _t.Iterable['ParsedLine']) -> int:
		...
		"""
		# Empty lines and lines with text are treated differently:
		lines_with_text = [x for x in lines if not x.is_empty]
		lines_empty = [x for x in lines if x.is_empty]
		for l in lines_empty:
			l.indent = 0

		if not lines_with_text:
			return 0
		common_indent = min(x.indent for x in lines_with_text)
		cls.offset_indent(lines_with_text, -common_indent)
	...
	def offset_indent(
		...
		recreation of :class:`ParsedLine` instances and the iterable itself.
		"""
		if (offset == 0 and not clamp_negative) or not lines:
			return
		if clamp_negative:
			for line in lines:
	...
	def block_type(self) -> BlockType:
		...
		if self.bullet and self.number:
			raise ValueError(f"The line is both a bulleted and numbered list item: {self}")
		if bool(self.bullet or self.number) != self.is_list:
			raise ValueError(f"The line is marked as list, and yet it has neither bullet nor number: {self}")
		if not self.is_list:
